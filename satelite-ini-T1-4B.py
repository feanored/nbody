...
satelite = sphere(pos=vector(2.1e7, 0,0), radius=2e6,color=color.yellow) # cria um satélite amarelo
                                                                          # 
...
scene.autoscale = 0  # mantém a escala da janela fixa (não atualiza automaticamente) 
# parr = arrow(color=color.green,shaftwidth=1e6) # seta para representar o momento 
escalap=0.5 # fator de escala para a seta do momento 
# Farr = arrow(color=color.red,shaftwidth=1e6) # seta para representar a força 
escalaF=5e3 # fator de escala para a seta da força 
...
#número de vezes que a coordenada y do satélite troca de sinal:
	...
	#limita a execução a 2 períodos completos (4 meias voltas)	
	N=4	
	if contador==N:
